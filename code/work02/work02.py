...
targetx = np.array([20, 160])
targety = np.array([180, 90])
obstaclex = np.array([[40, 60, 100, 60],
                      [140, 180, 160, 120],
                      [30, 50, 100, 80],
                      [120, 170, 140]])
obstacley = np.array([[140, 120, 140, 160],
                      [100, 170, 180, 160],
                      [40, 30, 40, 80],
                      [40, 40, 80]])
dx = np.array([[20, 40, -40, -20],
               [40, -20, -40, 20],
               [20, 50, -20, -50],
               [50, -30, -20]])
dy = np.array([[-20, 20, 20, -20],
               [70, 10, -20, -60],
               [-10, 10, 40, -40],
               [0, 40, -40]])
popularity = 20  # 粒子群数量
...
def internal(point):
    ...
    for i in range(len(obstaclex)):
        for j in range(len(obstaclex[i])):
            tempx.append(point[0] - obstaclex[i][j])
            tempy.append(point[1] - obstacley[i][j])
        inx.append(tempx)
        iny.append(tempy)
        tempx = []
        tempy = []
    for i in range(len(inx)):
        for j in range(len(inx[i])):
            if inx[i][j]*dy[i][j]-iny[i][j]*dx[i][j] < 0:
                return 0
    return 1
...
class Particle:
    def __init__(self):
        ...
        self.y = []
        for i in range(3):
            self.x.append(np.random.uniform() * 200)
            self.y.append(np.random.uniform() * 200)
    ...
    def solution(self):
        ...
        for i in range(len(self.allx)):
            self.state.append(internal([self.allx[i], self.ally[i]]))
    ...
    def cost(self):
        ...
        for i in range(length - 1):
            ...
            violation = self.state * 10000

        self.fitness = distance + violation
